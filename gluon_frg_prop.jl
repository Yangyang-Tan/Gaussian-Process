...
p0_gluon_sparse = p0_gluon[1:4:end]
G_gluon_sparse = G_gluon[1:4:end]
...
fx_gluon_f1 = f_gluon(x_gluon_f1, 1e-4)
...
function NLL_fun2(σ, l)
    C_α = Ctilde_RBF_fun.(p0_gluon_sparse', p0_gluon_sparse; σ=σ, l=l)
    C_α .= C_α + 1e-1 * I(length(p0_gluon_sparse))
    return 0.5 * ⋅(G_gluon_sparse_error', inv(C_α), G_gluon_sparse_error) +
           0.5 * logdet(C_α) +
           0.5 * length(p0_gluon_sparse) * log(2π)
end

@time NLL_fun(0.0001, 0.005)
@time NLL_fun2(0.0001, 0.005)

NLL_sigma = exp.(range(log(0.0001), log(1000); length=50));
NLL_l = exp.(range(log(0.005), log(1000); length=50));

Nll_result = zeros(length(NLL_sigma), length(NLL_l));

NLL_fun(0.01, 0.01)

@qthreads for i in 1:50
    for j in 1:50
        Nll_result[i, j] = NLL_fun2(NLL_sigma[i], NLL_l[j])
    end
end

heatmap(NLL_sigma, NLL_l, Nll_result; xaxis=:log, yaxis=:log, xlabel="sigma", ylabel="l")
